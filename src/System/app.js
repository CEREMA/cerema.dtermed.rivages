App = {
    init: function(app, server) {
        ...
        app.get('/geojson', function(req, res) {
            if (req.query['tkid']) var sql = '(select ST_AsGeoJSON(SHAPE) from import where tkid="' + req.query['tkid'] + '")';
            if (req.query['ofid']) var sql = '(select ST_AsGeoJSON(SHAPE) from import_segments where OGR_FID=' + req.query['ofid'] + ')';
            if (!sql) {
                res.status('404').send('Not found.');
            } else {
                res.writeHead(200, { "Content-Type": "application/json" });
                App.using('db').query('rivages', sql, function(e, r) {
                    for (var el in r[0]) var info = {
